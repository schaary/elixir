...
defmodule Mix.Dep.Loader do
  ...
  defp rebar_dep(%Mix.Dep{app: app} = dep, children, manager) do
    Mix.Dep.in_dependency(dep, fn _ ->
      ...
      extra  = Dict.merge(dep.extra, Dict.take(config, [:sub_dirs, :overrides]),
                          fn _, a, b -> a ++ b end)
      deps   = if children do
        ...
        rebar_children(app, config, extra[:overrides] || [], manager)
      end
  ...
  defp rebar_children(app, root_config, overrides, manager) do
    from = Path.absname("rebar.config")
    Mix.Rebar.recur(root_config, fn config ->
      deps = Mix.Rebar.deps(app, config, overrides)
      Enum.map(deps, fn dep ->
        %{to_dep(dep, from, manager) | extra: [overrides: overrides]}
      end)
    end) |> Enum.concat
  end
